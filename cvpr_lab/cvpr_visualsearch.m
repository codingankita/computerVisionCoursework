...
%% 4) Calculate PR
    pr_vals=zeros([1, NIMG]);
    rc_vals=zeros([1, NIMG]);
    ...
    for i=1:NIMG
        ...
        recall = true_positive / categories(1,qc);
    ...
    %% 5) calculate AP
    average_precision = sum(pr_vals .* cnt) / unique_categories(1,run);
    avp_val(run) = average_precision;

     %% 6) plot PR curve
    figure(1)
    plot(rc_vals, pr_vals);
    hold on;
    title('PR Curve');
    xlabel('Recall');
    ylabel('Precision');
